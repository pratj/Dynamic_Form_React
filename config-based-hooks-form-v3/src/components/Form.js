...
function Form({ cardInfo, setOpenPopup }) {
  ...
  const onSubmit = (data) => {
    const userLocation = geoLocation.loaded
      ? geoLocation
    ...
    axios
      .post(`${process.env.REACT_APP_BASE_URL}/api/response`, finalData)
      .then((response) => {
        const quoteData = {
          category: cardInfo.category,
          product: cardInfo.product,
  ...
  useEffect(() => {
    axios
      .get(
        `${process.env.REACT_APP_BASE_URL}/api/config/category/${cardInfo.category}/product/${cardInfo.product}`
      )
      .then((response) => {
